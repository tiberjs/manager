import { describe, expect, test } from "vitest";
import { Container, inject, type ResolutionGraph, scoped, token } from "../src/index.js";

function namedEdges(graph: ResolutionGraph): string[] {
  const names = new Map(graph.nodes.map(({ id, name }) => [id, name]));

  return graph.edges
    .map(({ from, to }) => {
      expect(names.has(from), "edge source must be an active graph node").toBe(true);
      expect(names.has(to), "edge target must be an active graph node").toBe(true);

      return `${names.get(from)} -> ${names.get(to)}`;
    })
    .sort();
}

function nodeId(graph: ResolutionGraph, name: string): number {
  const matches = graph.nodes.filter((node) => node.name === name);
  expect(matches.length, `expected one ${name} node`).toBe(1);

  return matches[0]!.id;
}

describe("resolution graph", () => {
  test("records every consumer of an already cached shared dependency", async () => {
    const container = new Container();
    class Shared {}
    class First {
      readonly shared = inject(Shared);
    }
    class Second {
      readonly shared = inject(Shared);
    }

    const shared = container.resolve(Shared);
    expect(container.resolve(First).shared).toBe(shared);
    expect(container.resolve(Second).shared).toBe(shared);
    container.resolve(Second);

    const graph = container.resolutionGraph();

    expect(graph.nodes.map(({ name }) => name).sort()).toStrictEqual(["First", "Second", "Shared"]);
    expect(namedEdges(graph)).toStrictEqual(["First -> Shared", "Second -> Shared"]);

    await container[Symbol.asyncDispose]();
  });

  test("distinguishes identical class names, opaque tokens, and aliases by identity", async () => {
    const container = new Container();
    const FirstStore = class Store {};
    const SecondStore = class Store {};
    const firstAlias = token<object>("Store");
    const secondAlias = token<object>("Store");
    const firstConsumer = token<object>("first");
    const secondConsumer = token<object>("second");

    container.provide(firstAlias, () => inject(FirstStore));
    container.provide(secondAlias, () => inject(SecondStore));
    container.provide(firstConsumer, () => inject(firstAlias));
    container.provide(secondConsumer, () => inject(secondAlias));

    container.resolve(FirstStore);
    container.resolve(SecondStore);

    expect(container.resolve(firstConsumer)).toBe(container.resolve(FirstStore));
    expect(container.resolve(secondConsumer)).toBe(container.resolve(SecondStore));
    expect(container.resolve(firstConsumer)).not.toBe(container.resolve(secondConsumer));

    const graph = container.resolutionGraph();
    const first = nodeId(graph, "first");
    const second = nodeId(graph, "second");
    const dependencyOf = (id: number): number => {
      const edges = graph.edges.filter(({ from }) => from === id);
      expect(edges.length).toBe(1);
      return edges[0]!.to;
    };

    const firstAliasId = dependencyOf(first);
    const secondAliasId = dependencyOf(second);
    const storeIds = [
      firstAliasId,
      secondAliasId,
      dependencyOf(firstAliasId),
      dependencyOf(secondAliasId),
    ];

    expect(new Set(storeIds).size).toBe(4);
    expect(
      graph.nodes
        .filter(({ name }) => name === "Store")
        .map(({ id }) => id)
        .sort(),
    ).toStrictEqual(storeIds.sort());
    expect(graph.nodes.length).toBe(6);
    expect(graph.edges.length).toBe(4);

    await container[Symbol.asyncDispose]();
  });

  test("nested resolution across independent roots never creates cross-root edges", async () => {
    const first = new Container();
    const second = new Container();
    const external = token<object>("external");
    const dependency = token<object>("dependency");
    const consumer = token<object>("consumer");

    second.provide(dependency, () => ({}));
    second.provide(external, () => inject(dependency));
    first.provide(consumer, () => second.resolve(external));

    expect(first.resolve(consumer)).toBe(second.resolve(external));
    expect(first.resolutionGraph().nodes.map(({ name }) => name)).toStrictEqual(["consumer"]);
    expect(first.resolutionGraph().edges).toStrictEqual([]);
    expect(namedEdges(second.resolutionGraph())).toStrictEqual(["external -> dependency"]);

    await first[Symbol.asyncDispose]();
    await second[Symbol.asyncDispose]();
  });

  test("child-local providers have distinct ownership and disposal removes both incoming and outgoing edges", async () => {
    const root = new Container();
    const child = root.child();
    const sibling = root.child();
    const shared = token<object>("shared");
    const local = token<object>("local");
    const consumer = token<object>("consumer");
    const borrowed = token<object>("borrowed");
    const events: string[] = [];

    root.provide(shared, () => ({}));
    root.provide(local, () => ({}));
    const rootLocal = root.resolve(local);
    root.resolve(shared);

    child.provide(local, () => ({ shared: inject(shared) }));
    sibling.provide(local, () => ({ shared: inject(shared) }));
    child.provide(consumer, () => inject(local));
    sibling.provide(consumer, () => inject(local));
    root.provide(borrowed, () => child.resolve(consumer));

    expect(root.resolve(borrowed)).toBe(child.resolve(consumer));
    expect(child.resolve(consumer)).not.toBe(sibling.resolve(consumer));
    expect(child.resolve(local)).not.toBe(rootLocal);

    child.defer(() => {
      events.push("child disposed");
    });

    const graph = root.resolutionGraph();

    expect(child.resolutionGraph()).toStrictEqual(graph);
    expect(graph.nodes.filter(({ name }) => name === "local").length).toBe(3);
    expect(namedEdges(graph)).toStrictEqual([
      "borrowed -> consumer",
      "consumer -> local",
      "consumer -> local",
      "local -> shared",
      "local -> shared",
    ]);

    await child[Symbol.asyncDispose]();

    expect(events).toStrictEqual(["child disposed"]);
    expect(
      root
        .resolutionGraph()
        .nodes.map(({ name }) => name)
        .sort(),
    ).toStrictEqual(["borrowed", "consumer", "local", "local", "shared"]);
    expect(namedEdges(root.resolutionGraph())).toStrictEqual([
      "consumer -> local",
      "local -> shared",
    ]);
    expect(root.resolve(local)).toBe(rootLocal);

    await sibling[Symbol.asyncDispose]();

    expect(
      root
        .resolutionGraph()
        .nodes.map(({ name }) => name)
        .sort(),
    ).toStrictEqual(["borrowed", "local", "shared"]);
    expect(root.resolutionGraph().edges).toStrictEqual([]);

    await root[Symbol.asyncDispose]();

    expect(root.resolutionGraph()).toStrictEqual({ nodes: [], edges: [] });
  });

  test("inline resources record cached edges and root disposal cannot be repopulated by a surviving child", async () => {
    const root = new Container();
    const child = root.child();
    const resource = token<object>("resource");
    const consumer = token<object>("consumer");
    const later = token<object>("later");
    let disposed = 0;
    const value = child.use(
      resource,
      () => ({}),
      () => {
        disposed++;
      },
    );

    expect(
      child.use(consumer, () => scoped(resource, () => expect.unreachable("must use cache"))),
    ).toBe(value);
    expect(namedEdges(root.resolutionGraph())).toStrictEqual(["consumer -> resource"]);

    await root[Symbol.asyncDispose]();

    expect(disposed).toBe(0);
    expect(child.resolve(resource)).toBe(value);

    child.use(later, () => ({}));

    expect(root.resolutionGraph()).toStrictEqual({ nodes: [], edges: [] });
    expect(child.resolutionGraph()).toStrictEqual({ nodes: [], edges: [] });

    await child[Symbol.asyncDispose]();

    expect(disposed).toBe(1);
  });

  test("failed attempts remain diagnostic evidence without contaminating later resolutions", async () => {
    const container = new Container();
    const failing = token<object>("failing");
    const dependency = token<object>("dependency");
    const later = token<object>("later");
    const failure = new Error("construction failed");
    let attempts = 0;

    container.provide(dependency, () => ({}));
    container.provide(failing, () => {
      const value = inject(dependency);
      if (++attempts === 1) {
        throw failure;
      }

      return value;
    });
    container.provide(later, () => inject(dependency));

    let constructionFailure: unknown;
    try {
      container.resolve(failing);
    } catch (error) {
      constructionFailure = error;
    }

    expect(constructionFailure).toBe(failure);

    expect(namedEdges(container.resolutionGraph())).toStrictEqual(["failing -> dependency"]);

    expect(container.resolve(later)).toBe(container.resolve(failing));

    expect(namedEdges(container.resolutionGraph())).toStrictEqual([
      "failing -> dependency",
      "later -> dependency",
    ]);

    container.defer(() => {
      throw failure;
    });
    const disposal = container[Symbol.asyncDispose]();
    await expect(disposal).rejects.toBe(failure);

    expect(container.resolutionGraph()).toStrictEqual({ nodes: [], edges: [] });
  });
});
